...
void CheckActiveNode(struct node *root)
{
    struct node *temp=root;
    while(temp!=NULL)
    {
        if(fork()==0)
        {
            if(CheckActive(temp->port))
                printf("%s node is active at %d\n",temp->name,temp->port);
            exit(0);
        }
        else
        {
...
int main(char argc,char *argv[]) 
{
    ...
    while(1)
    {
            ...
            while(temp!=NULL)
            {
                if(fork()==0)
                {
                    ...
                    *c++='\0';
                    ...
                    command[i]='\0';
                    strcpy(node,temp->name);
                    printf("Calling %s:At port %d\n",command,temp->port);
                    client(command,temp->port);
                    exit(0);
                }
                else
                {
		...
		else
		{
            ...
            {
               res = client(buff,ReturnPort(root,argv[2]));
               switch (res)
               {
               case 0:
                  continue;
